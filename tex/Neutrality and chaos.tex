...
%TODO Unclear
%TODO What parameter is that?
Our target is to estimate the probability of reaching one of such chaotic attractors under different assumptions about intraspecific competition. In order to achieve this, we simulated several ecosystems with different initial conditions \todo[caption = {Unclear}]{did you run different initial conditions to find all attractors of a single drawn parameter set, or did you take both the initial conditons and the parameters at random for each run?} and predation coefficients, but drawn using the same competition parameter.
...
In the spirit of mathematical modelling, we chose the simplest realization required for the effects to be observed. We didn't use Allee effect, nor noise, and the functional form of each term has been chosen to account for satiation and saturation in the simplest possible ways. The choice of a two-level model may seem in contradiction with the pursue of simplicity, but actually it is a fundamental requirement for the effect under research to take place. In the absence of a predator level, chaos will never develop in a model with neutral competition. The reason for this is that if all interactions become equally strong, the differences among species at the same trophic level fade out. This makes labeling each species meaningless, and thus the prey-only system can be reduced just to one differential equation, that of the total prey population (see Appendix for details). Using the Poincaré-Bendixson theorem, it can be proven that chaos autonomous systems with less than $3$ dimensions cannot exhibit chaos \cite{Strogatz1994}. 
...
It is important to note that, in models like this, all deaths at the prey's level are due to predation. All deaths are \textit{invested} in consumer's growth.  Therefore the following relation must be true:
...
It is difficult to overestimate the importance of restriction \ref{eq:InnerEnergyFlowMulti} (or equivalently, \ref{eq:AllEnergyFlowMulti}). We need it to build realistic functional forms for the coupling terms. As the coupled terms that are related to predation, those terms are the very core of this models.
...
%TODO Is V really needed?
\todo[inline]{Is V really needed?}
