...
Our main target is to estimate the probability of reaching a chaotic attractor under different intensities of neutrality. In order to achieve this, we'll simulate several ecosystems sharing the same competition parameter , but varying the initial conditions and the predation parameters. Due to the complexity of the system, we face the problem as a numerical simulation. For the sake of reproducibility, we provide a \textit{GitHub} link to the scripts used.
...
The outcome of the numerical experiment described above are two lists of \textit{votes} for chaos per value of $ \epsilon $, one list corresponding to the maximum Lyapunov test, and the other to the z1 Gottwald-Melbourne test [CITATION NEEDED]. This list can be used to estimate the probability of chaos for each value of $ \epsilon $ via the classical frequentist interpretation of probability, that is, as the ratio of attractors found to be chaotic.
