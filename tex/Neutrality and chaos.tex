...
Fascination for biodiversity is one of the main motivations for studying ecology. Even very young children feel the joy of learning about different species, so no prior knowledge of biology seems to be a requirement for being sensitive to the amazing variety of life. Scientific knowledge increases this sense of wonder even more as there is a big mystery: why are there so many species?\todo[inline, caption={Clarify the paradox}]{\textbf{Egbert}: Why are there so many species is for me not the complete paradox. \textbf{Pablo}: I like this "poetical" introduction. Maybe I can properly define the paradox in next paragraph, after Gause's law? \textbf{Egbert}: Yes ok to move it to later, now it is better.}
...
The mystery comes into scene together with the competitive exclusion principle\cite{Hardin1960}, sometimes referred to as Gause's law. This principle, which some authors trace back to Charles Darwin's \textit{On the origin of the species}\cite{Darwin1859}, is one of the classical keystones of ecology. The principle states that \textit{"for each niche only one species will dominate in the long run, out-competing the rest"}. Most competition models satisfy this principle, and it has been observed experimentally under laboratory conditions. On the other hand, complete exclusion rarely occurs in nature\cite{Huston1979}, and the huge biodiversity in rather homogeneous systems such as pelagic ecosystems is a noticeable counterexample\todo{Cite Hutchinson}. This contradiction is known as the biodiversity paradox. The paradox can be rephrased as \textit{Why are there so many species if there are so few niches?}
%TODO Cite Hutchinson

%% Brief discussion about the alternative hypotheses
% Introduction and hypotheses based in non-autonomous systems
There are several hypotheses to explain the paradox. For instance, Hutchinson\cite{Hutchinson} suggests that the number of niches of species can be higher due to a separation in time of competition. As an example he reports observations of two sympatric species of beetles competing for the same resource, but not simultaneously because of having different breeding seasons.\todo{I expect directly some hypotheses} In a later, influential paper\cite{Hutchinson1961}, Hutchinson proposes the possibility that more species can coexist due to external, time-dependent environmental changes, being seasonal factors an obvious example. If the characteristic times of this environmental changes are fast enough, the ecosystem is prevented to reach an equilibrium. A pioneering quantitative discussion about ecosystems subject to periodically driven external perturbations can be found in \cite{Huston1979} \todo[inline, caption={Recent literature}]{\textbf{Egbert}: There are also some recent papers showing this. I was reviewer in this one: Lumpy species coexistence arises robustly in fluctuating resource environments
Athanasia Sakavara, George Tsirtsis, Daniel L. Roelke, Rebecca Mancy and Sofie Spatharis PNAS January 23, 2018. 115 (4) 738-743; published ahead of print December 20, 2017. https://doi.org/10.1073/pnas.1705944115.}
%TODO Recent literature
%TODO I expect directly some hypotheses
...
All the previous hypotheses refer to the time domain. Regarding the spatial dimension, the inhomogeneity of ecosystems, and the possibility of migration between them has been pointed out as another possible explanation of the paradox \cite{Tilman1994}. \todo[inline, caption={Broken flow}, color=green!25]{\textbf{Tobias}: Flow broken here. High risk of the reviewer stopping reading! Why are all the previous explanations not sufficient? Why does neutrality deserve attention? \textbf{Pablo}: I'm not sure if I agree. \textbf{Egbert}: I agree with Tobias that the flow is broken a bit. You just need to make the connection with the previous more explicit. (certainly not delete this as it is essential for our story)}
...
A rather radical other explanation for high diversity is \textit{Hubbell's neutral competition theory} \cite{Hubbell2001}. Here it is assumed that similar species inside an ecological community have identical \textit{per capita} rates of birth, death, reproduction, etc. In those models, the long term differences between species are a result of stochastic processes. Despite its counter-intuitive and controversial foundations, neutral models have been successfully applied to populations of rainforest trees \cite{Hubbell2001}. Some evidence about the counter-intuitive fact that similarity can give rise to coexistence can be found in \cite{Scheffer2006}.

%TODO 
\todo[inline, caption={Yachi's insurance hypothesis}]{\textbf{Egbert}: In neutral models the species can coexist because they are identical. But this is rather unrealistic, as species will for instance react differently to perturbations (response diversity). Maybe introduced in \textit{Yachi, S. and M. Loreau. 1999}. \textbf{Pablo}: I don't fully understand how to fit Yachi's insurance hypothesis here. Can you please check if this is the paper you meant? \textbf{Egbert}: Maybe it is not needed here. We could also move this to the discussion or so. But I think response diversity is essential for insurance: quote: "The strength of these insurance effects is determined by three factors: (i) the way ecosystem productivity is determined by individual species responses to environmental fluctuations, (ii) the degree of asynchronicity of these responses, and (iii) the detailed form of these responses."}
%In neutral models the species can coexist because they are identical. But this is rather unrealistic, as species will for instance react differently to perturbations (response diversity). Maybe introduced in this publcation (please check): Yachi, S. and M. Loreau. 1999. Biodiversity and ecosystem productivity in a fluctuating environment: The insurance hypothesis. Proceedings of the National Academy of Sciences of the United States of America 96:1463-1468 {Yachi1999}
...
%TODO Wrong argument
Most of the previously mentioned theories involve competition only, whereas in many ecosystems predation is also relevant. It is known from models that the inclusion of a multi-species predation trophic level increases the probability of chaos \cite{Dakos2009b}, and may thus contribute to increase the number of coexisting species\todo[caption = {Wrong argument}]{The arguments should be ecological}. In the present paper we explore the links between the heterogeneity at the competition level of a two trophic levels model (see figure \ref{fig:Network}) and the type of long term dynamics exhibited (see figure \ref{fig:TimeSeries}).\todo[color=blue!25, caption = {Remove figures}]{Remove figures. Are we sure?}

%TODO Egbert suggests removing these figures

%TODO Maybe use this in the conclusions or summary. It is indeed very similar to abstract
% Our simulations show that the likelihood of chaotic dynamics increases when heterogeneity at the competitors' trophic level is minimal. That is, that neutrality in combination with multiple predators may promote chaos. The competitive exclusion principle, based in equilibrium assumptions, is thus less likely to be applicable to neutral ecosystems. As a result, these systems have more chances of holding a higher amount of coexisting species than non neutral ones.
...
A secondary source for prey's growth in our model will be a small constant immigration term $ f $, representing immigration from neighboring areas. We include this term to avoid complete extinctions and unrealistic heteroclinic cycles \cite{Scheffer2004}.
...
\end{equation}
%TODO Rename feeding
\todo[inline, caption = {Rename feeding}]{Feeding is confusing as the sum of predation should be the sum of the feeding. Possible new name: CrossGrowth}
...
Depending on the parameters and the initial conditions, this system can give rise to three types of asymptotic behavior, each of them roughly corresponding to a different type of attractor. The easier one, corresponding to a stable point attractor, generates a constant species composition. Cyclic attractors give rise to periodically changing species composition. Beyond fixed points and cycle attractors, there is a wide variety of more complex possible behaviors\todo{Example}. As a simplification, we'll refer to all them as chaotic.
\todo[inline, caption = {Think about pseudochaos}]{I think it is better to move this text to the numerical experiments where you  talk about the 0-1 test. I don't really understand which other complex behaviours we call chaos (can you explain that if you keep the text), isnt it just defined as a positive 0-1 test?}
%TODO Think about pseudochaos
...
Our target is to estimate the probability of reaching a chaotic attractor under different assumptions about intraspecific competition. In order to achieve this, we simulated several ecosystems with different initial conditions and relative predation intensities, but sharing the same competition parameter.
...
For the sake of reproducibility, we provide a \textit{GitHub} link to the analysis scripts used\footnote{https://github.com/PabRod/Chaos-and-neutrality}. For further information, please refer to the \textit{read me} file.
...
%TODO Move sentence to discussion
There's another local maximum for $ \epsilon = -1 $. This means that ecosystems with weak competition coupling also promote chaos. We consider this a reasonable result, as predation is the driver of chaos\todo[caption = {Move sentence to discussion}]{I think this is not wise to put here as it seems that the result is not surprising. Better to explain the reason of chaos. But maybe better in the discussion?}. Between both local maxima there was obviously a local minimum whose exact position differed between experiments.
...
The asymptotic dynamics of our model are affected by how strong intraspecific competition compared to interspecific competition is. Particularly, the closer to neutrality the  competition our food web is, the higher are the chances of developing chaotic behaviour. This suggests that in a system with predation, near-neutrality may increase the probability of complex dynamics if the species are not equally prone to predation. Provided the competitive exclusion principle rests on the assumption of equilibrium, near-neutrality reduces the chances of the principle to be applicable, improving the chance for a higher number of coexisting species. Additionaly, this observation suggests that the hypothesis of non-equilibrium and Hubbell's hypothesis of neutrality are not completely independent.

% Limitations
In the spirit of mathematical modelling, we chose the simplest realization required for the effects to be observed. We didn't use Allee effect, nor noise, and the functional form of each term has been chosen to account for satiation and saturation in the simplest possible ways. The choice of a two-level model may seem in contradiction with the pursue of simplicity, but actually it is a fundamental requirement for the effect under research to take place. In the absence of predation, chaos will never develop in a model with neutral competition. The reason for this is that if all interactions become equally strong, the differences among species at the same trophic level fade out. This makes labeling each species meaningless, and thus the prey-only system can be reduced just to one differential equation, that of the total population. Using the Poincaré-Bendixson theorem, it can be proven that chaos cannot be developed in autonomous systems with less than $3$ dimensions\cite{Strogatz1994}.

% Predation parameters
We used the simplest predation network, that is, one with random prey preference for the predators. A possible continuation of this paper could be studying the effect of different types of predation (generalist, specialist, neutral) on the asymptotic dynamics.
%TODO Chaos and predation
\todo[inline, caption={Chaos and predation}]{Add some discussion about this assumption.  It is clear that the chaos is created here so this deserves more attention. It is essential for chaos that the predators have a different preference and that the predation is rather efficient: in a two species case it predator-prey cycles should be possible.}

%TODO Network architecture
\todo[inline, caption={Network architecture}]{You can also discuss network architectures of foodwebs (they seem to be modular) (see Thébault, E. and C. Fontaine. 2010. Stability of ecological communities and the architecture of mutualistic and trophic networks. Science 329:853-856.)}

% Competition parameters
The competition parameters were also drawn randomly. It is known that the asymptotic behaviour of this kind of systems can be very sensitive to the parameters choice. In particular, introducing correlations between parameters can greatly modify the probabilities of chaotic attractors to be reached (see for instance \cite{Huisman2001}, in response to the letter \cite{Schippers2001}). In the present paper we didn't introduce any correlation, i.e., all our random parameters were drawn independently from the others. Studying the effect of different physiological scenarios (in the sense of \cite{Huisman2001}, that is, constrains between the parameters) on the probabilities of chaos could be a straightforward continuation to this paper.
...
%TODO Effects of symmetry
\todo[inline, caption={Effects of symmetry}]{Good chance that a reviewer says: nice these suggestions, but why do they not show it here?}

% The parameter epsilon
%TODO Biological interpretation of parameter
Interestingly enough, a relatively small number of replicates suffices to generate a remarkably clear maximum in the probability of chaos for neutral competition. The position of this maximum is independent of the number of interacting species. This suggests that the parameter $ \epsilon $, originally motivated by numerical convenience, may have a biological interpretation by itself\todo[caption = Biological interpretation of parameter]{This is not clear}.

% Chaos detection
%TODO Chaotic transients
Numerical detection of chaos has fundamental limitations, that can be boiled down to the fact that, in general, numerical methods cannot distinguish robustly between long \todo[color=blue!25, caption = {Chaotic transients?}]{\textbf{Egbert}: chaotic? \textbf{Pablo}: I would say only attractors can be chaotic, so that excludes transients} transients and genuine chaos. In the present paper three parallel approaches were followed: Lyapunov exponents estimation, Gottwald-Melbourne \textit{0-1} test and visual inspection\todo{Wrong section}. Despite differences in the exact probabilities, the three of them led us to the same qualitative conclusions. We found the results of the Gottwald-Melbourne test most consistent with visual inspection. We called chaotic all those time series complicated enough to trigger both the Gottwald-Melbourne test and the visual inspection tests. We think that this approach to chaos detection, despite being open to improvement, suffices to hold the biological conclusions.
...
%
%The numerical experiment described in subsection \ref{subsec:NumericalExperiment} can be summarized as a the following flow chart:
%
%\begin{figure}[H]
%	\begin{center}
...
\todo[inline, caption={Online appendix}]{Decide how to split the information into main paper + appendix. \textbf{Egbert}: I think you need to make an appendix explaining the relation between Lyapunov and the other indices. Maybe some additional figures (if you have them?) In the results section you should refer to those extra results. }
%TODO Past tense
\todo[inline, caption={Past tense}]{Use past tense along all the paper}
%TODO Too much suggestions
\todo[inline, caption={Too much suggestions}]{It is maybe dangerous to suggest too much additional analysis in the discussion, as reviewers will wonder why you did not do it here.}
