...
%TODO Wrong argument
Most of the previously mentioned theories involve competition only, whereas in many ecosystems predation is also relevant. It is known from models that the inclusion of a multi-species predation trophic level increases the probability of chaos \cite{Dakos2009b}, and may thus contribute to increase the number of coexisting species\todo[caption = {Wrong argument}]{The arguments should be ecological}. In the present paper we explore the links between the heterogeneity at the competition level of a two trophic levels model and the type of long term dynamics exhibited.
...
\end{equation}
%TODO Rename feeding
\todo[inline, color = yellow!25, caption = {Rename feeding}]{Feeding is confusing as the sum of predation should be the sum of the feeding. Possible new name: CrossGrowth}
...
There's another local maximum for $ \epsilon = -1 $. This means that weak competition coupling also promotes chaos in our model.

Between both local maxima there was obviously a local minimum whose exact position differed between experiments.
...
The asymptotic dynamics of our model are affected by how strong intraspecific competition compared to interspecific competition is. We find particularly interesting the fact that the closer to neutrality the competition in our food web is, the higher are the chances of developing chaotic behaviour. This suggests that in a system with predation, near-neutrality may increase the probability of complex dynamics if the species are not equally prone to predation. Provided the competitive exclusion principle rests on the assumption of equilibrium, near-neutrality reduces the chances of the principle to be applicable, improving the chance for a higher number of coexisting species. Additionaly, this observation suggests that the hypothesis of non-equilibrium and Hubbell's hypothesis of neutrality are not completely independent.

% Maximum at $ \epsilon = -1 $
Additionally, our model shows another local maximum for the probability of chaos for weak competition coupling. We consider this a reasonable result, as predation is the driver for chaos. \todo[color=yellow!25]{I don't fully like this sentence}
...
\todo[inline, color=yellow!25, caption={Effects of symmetry}]{\textbf{Egbert}: Good chance that a reviewer says: nice these suggestions, but why do they not show it here? \textbf{Pablo}: remove or do?}

% The parameter epsilon
%TODO Biological interpretation of parameter
Interestingly enough, a relatively small number of replicates suffices to generate a remarkably clear maximum in the probability of chaos for neutral competition. The position of this maximum is independent of the number of interacting species. This suggests that the parameter $ \epsilon $, originally motivated by numerical convenience, may have a biological interpretation by itself\todo[color = yellow!25, caption = Biological interpretation of parameter]{\textbf{Egbert}: This is not clear. \textbf{Pablo}: Remove?}.

% Chaos detection
%TODO Chaotic transients
Numerical detection of chaos has fundamental limitations, that can be boiled down to the fact that, in general, numerical methods cannot distinguish robustly between long \todo[color=yellow!25, caption = {Chaotic transients?}]{\textbf{Egbert}: chaotic? \textbf{Pablo}: I would say only attractors can be chaotic, so that excludes transients} transients and genuine chaos. We think that our approach to chaos detection, despite being open to improvement, suffices to hold the biological conclusions.
...
%TODO Past tense
\todo[inline, color = gray!25, caption={Past tense}]{Use past tense along all the paper}
