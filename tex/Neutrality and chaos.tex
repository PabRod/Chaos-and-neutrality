...
\usepackage[disable, colorinlistoftodos]{todonotes} % Use disable instead of draft to hide todo notes
...
All the previous hypotheses refer to the time domain. Regarding the spatial dimension, the inhomogeneity of ecosystems, and the possibility of migration between them has been pointed out as another possible explanation of the paradox \cite{Tilman1994}. \todo[inline, color=yellow!25, caption={Broken flow}, color=yellow!25]{\textbf{Tobias}: Flow broken here. High risk of the reviewer stopping reading! Why are all the previous explanations not sufficient? Why does neutrality deserve attention? \\ \textbf{Egbert}: I agree with Tobias that the flow is broken a bit. You just need to make the connection with the previous more explicit. (certainly not delete this as it is essential for our story). \\ \textbf{Pablo}: Maybe: \textit{"A rather radical other explanation difficult to fit in the two previous groups..."}}
...
\end{equation}
%TODO Rename feeding
\todo[inline, color = yellow!25, caption = {Rename feeding}]{\textbf{Egbert}: Feeding is confusing as the sum of predation should be the sum of the feeding. Possible new name: CrossGrowth. \\ \textbf{Pablo}: being the sum up to assimilation constant maybe it is clearer to take the constant out of the equation.}
...
\todo[inline, color=yellow!25, caption={Chaos and predation}]{\textbf{Egbert}: Add some discussion about this assumption.  It is clear that the chaos is created here so this deserves more attention. It is essential for chaos that the predators have a different preference and that the predation is rather efficient: in a two species case it predator-prey cycles should be possible. \textbf{Pablo}: These ideas are far from trivial. References will be required}

%TODO Network architecture
\todo[inline, color=yellow!25, caption={Network architecture}]{\textbf{Egbert}: You can also discuss network architectures of foodwebs (they seem to be modular) (see Thébault, E. and C. Fontaine. 2010. Stability of ecological communities and the architecture of mutualistic and trophic networks. Science 329:853-856.) \textbf{Pablo}: our model has a single, big cluster of competitors. How can we explain our choice?}

% Competition parameters
The competition parameters were also drawn from probability distributions. It is known that the asymptotic behaviour of this kind of systems can be very sensitive to the parameters choice. In particular, introducing correlations between parameters can greatly modify the probabilities of chaotic attractors to be reached (see for instance \cite{Huisman2001}, in response to the letter \cite{Schippers2001}). In the present paper we didn't introduce any correlation, i.e., all our random parameters were drawn independently from the others. Studying the effect of different physiological scenarios (in the sense of \cite{Huisman2001}, that is, constrains between the parameters) on the probabilities of chaos could be a straightforward continuation to this paper.
...
Numerical detection of chaos has fundamental limitations, that can be boiled down to the fact that, in general, numerical methods cannot distinguish robustly between long \todo[inline, color=yellow!25, caption = {Chaotic transients?}]{\textbf{Egbert}: chaotic? \\ \textbf{Pablo}: I would say only attractors can be chaotic, so that excludes transients. Maybe: \textit{"chaotic looking / complex / complicated transients"}} transients and genuine chaos. We think that our approach to chaos detection, despite being open to improvement, suffices to hold the biological conclusions.
