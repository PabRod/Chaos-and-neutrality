...
where $P_i(t)$ represents the biomass of prey $i$ at time $t$ and $C_j(t)$ the biomass of predator species $j$ at time $t$. The auxiliary function $g_j(P)$ is defined as:
...
The case of an underlying chaotic time series is the only one that generates a path that doesn't stay inside a bounded domain around the starting point. The z1 test uses the mean square displacement as a measure of this diffusion. The system is considered to be chaotic if the square displacement keeps growing for large times. If, on the contrary, it stays bounded, the test will consider the system not chaotic.
