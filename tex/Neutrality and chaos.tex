...
In the present paper we explore the links between neutral competition and the type of long term dynamics exhibited. Using weakly non-neutral models, our simulations show that the likelihood of chaos increases with neutrality at the competitors' trophic level.
...
\paragraph{}
This choice was made because these webs are the simplest realization of a system where neutrality doesn't lead to mathematical degeneration (see \ref{sec:Appendix}). Degeneration is prevented by the non-neutral predation rates breaking the excess of symmetry provided by neutrality. % TODO: maybe not needed in an ecology journal
...
The growth term is modelled as a multispecies logistic growth. The strength of the competition between species $ i $ and $ k $ is given by $ A_{ik} $. Those coefficients can be arranged as an $ n \times n $ matrix. So, for prey $ i $, we have:
...
The palatability of each prey species is given by $ S_{ij} $. Those coefficients can be arranged as an $ N \times n $ matrix. Being a multispecies model, we can define the auxiliary variable $ V_j $ as a sum of the prey's populations weighted by the palatability coefficients. Biologically, this represents the composition of the \textit{menu} of consumer $ j $:
...
By choosing $ -1 $ as a lower limit for $ \epsilon $ we exclude the possibility of negative numbers in the competition matrix. This coefficients of \textit{negative competition} can be understood as a simple approach to mutualism, and we wanted to focus only in classical competition.
...
\begin{enumerate}
	\item \label{GoToAttractor} Run the simulation for a sufficient time, in order to guarantee that a trajectory is on an attractor
	\item \label{RunInAttractor} Use the run in step \ref{GoToAttractor} as a starting point for a second, shorter run on the attractor
