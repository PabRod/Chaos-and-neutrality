	...
	\begin{center}
		\resizebox{\columnwidth}{!}{
			\begin{tabular}{|c|c|c|} \hline 
				...
				\hline 
				Two species & $	\begin{cases}\dot{P} = r P - g P C\\ \dot{C} = - l C + e g C P\end{cases}$ & $\begin{cases}\dot{P} = r(P) P - g(P) P C\\\dot{C} = - l C + e g(P) C P\end{cases}$ \\ 
				\hline 
				Multispecies & $\begin{cases} \dot{P_i} = r_i P_i - P_i \sum_{j = 1}^{n_C} g_j S_{ji} C_j & : i = 1..n_P \\ \dot{C_j} = - l_j C_j +  g_j e_j C_j \sum_{i = 1}^{n_P} S_{ji} P_i  & : j = 1..n_C \end{cases}$ & $\begin{cases} \dot{P_i} = r_i(P) P_i - P_i \sum_{j = 1}^{n_C} g_j(P) S_{ji} C_j & : i = 1..n_P \\ \dot{C_j} = - l_j C_j +  g_j(P) e_j C_j \sum_{i = 1}^{n_P} S_{ji} P_i  & : j = 1..n_C \end{cases}$  \\
			\hline  \end{tabular}}
	\end{center}
