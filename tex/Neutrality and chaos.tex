...
%TODO see also Huston, M. A. 1994. Biological Diversity: The Coexistence of Species on Changing Landscape. Cambridge University Press, Cambridge. Book.
...
%TODO not clear why I want to study: the links between neutral competition and the type of long term dynamics exhibited. Mention predation
...
We hypothesized that the feeding term will be linear in $ C_j $. The dependency on $V_j$ happens through a Holling type II functional response with half saturation constant $ H $ in order to account for consumer satiation: 
%TODO cite Edelstein Keshet

\begin{equation}
\label{eq:Feeding}
	Feeding_j = e g C_j \frac{V_j}{V_j+H} = e g C_j F_{2}(V_j; H)
\end{equation}
...
%TODO this can be rewritten in an equation-less way
...
%TODO Mention simulation time, step, time series length for the 0-1 test, ...
...
Additionally, we are implicitly assuming without a proof that chaotic ecosystems can have more species than non-chaotic ones.
